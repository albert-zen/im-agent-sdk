...
4. [Vision](VISION.md) for product purpose, non-goals, and the threshold for
   common abstractions.
5. [Architecture](ARCHITECTURE.md) for the implemented ownership and dependency map.
...
| `gateway/runtime.py`, `gateway/orchestration.py`, and `gateway/lifecycle.py` | [Gateway lifecycle design](components/gateway/lifecycle/design.md), [testing](components/gateway/lifecycle/testing.md) | public lifecycle, private bridge ordering, startup FIFO, rollback races, shutdown, and cleanup |
...
| `gateway/diagnostics.py` | [Gateway diagnostics design](components/gateway/diagnostics/design.md) and [testing](components/gateway/diagnostics/testing.md) | Gateway I1/I2/O1/O2, projection/startup aggregation, bounded fail-closed collection, focused owner identity, and clean-wheel smoke |
