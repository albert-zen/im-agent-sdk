...
- stable diagnostics aggregate Application, Channel, worker, and startup
  admission facts without exposing Thread/route/error identities or mutating
  runtime state;
- live-only created messages share route ordering and idempotent delivery but
  never advance authoritative completion checkpoints;
