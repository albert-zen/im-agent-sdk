...
history proof must consume the refs the checkout already fetched. Publication
is verified afterward by downloading the private asset through an authenticated
GitHub session, checking its digest, installing it in a fresh Python 3.13
environment, and comparing `imagent.__version__` with the release version.
