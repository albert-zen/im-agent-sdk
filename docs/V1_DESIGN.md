...
in the current native connection epoch, still exposes the stable native
session identity and one of the finite revisions authorized by that create or
an allowlisted non-Turn initialization notification, and has not yet crossed a
native-input dispatch fence or produced an allowlisted typed native Turn fact.
For only that state, a new checkpoint-free route's authoritative read
...
reconstruction fall back to normal strict history. An in-flight baseline that
captured valid evidence before an allowlisted Turn event may complete its
empty baseline; the already-subscribed live event remains queued and drains
only after the barrier opens. Existing routes, checkpointed recovery,
ambiguous history
