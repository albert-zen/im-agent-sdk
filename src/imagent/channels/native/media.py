...
MEDIA_LOCK_FILE_NAME = ".imagent-spool.lock"
...
class ImageMediaMaterializer(Generic[ImageReferenceT]):
    ...
    def __init__(
        ...
        *,
        ...
        self._retained_processes: set[Any] = set()
        self._tainted = False
        self._closed = False
...
class FileMediaMaterializer(ImageMediaMaterializer[FileReferenceT]):
    ...
    async def materialize(  # pyright: ignore[reportIncompatibleMethodOverride]
        self,
    ...
    async def _run_stage_worker(  # pyright: ignore[reportIncompatibleMethodOverride]
        self,
        ...
        while True:
            ...
            if lock_status == "acquired":
                ...
                for image in images:
                    ...
                    staged.append((image.content_type, str(final_path), len(image.data)))
    ...
    *,
    ...
    if image_references:
        if image_materializer is None:
            raise ValueError("image materializer is required for image references")
    ...
    if inbound.input_error is None and file_references:
        if file_materializer is None:
            raise ValueError("file materializer is required for file references")
