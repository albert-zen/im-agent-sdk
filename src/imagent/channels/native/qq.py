...
class QQChannelAdapter(BaseChannelAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self.media_materializer = media_materializer or QQMediaMaterializer(
            root=media_dir or Path(".imagent") / "channels" / "qq" / "inbound-media",
    ...
    def from_config(cls, *, config: dict[str, object], middleware):
        return cls(
            ...
            media_dir=Path(str(config.get("media_dir") or ".imagent/channels/qq/inbound-media")),
    ...
    def _parse_inbound_event(
        ...
        if quote is not None:
            text_parts.append(render_qq_quote_context(quote))
        normalized_text = "\n\n".join(text_parts)
    ...
    async def send_message(self, message: OutboundMessage) -> NativeDeliveryResult:
        ...
        for index, chunk in enumerate(split_text(message.text, limit=QQ_TEXT_LIMIT)):
            ...
            try:
                ...
                if not self._should_retry_plain_text(exc):
                    raise
                logger.warning(
                    "QQ markdown message failed; retrying as plain text",
                    exc_info=True,
                )
    ...
    def _queue_dispatch_event(
        ...
        try:
            ...
            raise RuntimeError(
                "QQ inbound queue is full; reconnecting before acknowledging messages"
            ) from None
    ...
    def _message_sequence(
        ...
        *,
        ...
        stable_part_id = delivery_id if part_index == 0 else f"{delivery_id}\0part:{part_index}"
        digest = hashlib.sha256(stable_part_id.encode("utf-8")).digest()
    ...
    def _acknowledge_duplicate_delivery(
        ...
        if not message.metadata.get("delivery_id") or not self._is_duplicate_message_error(exc):
            return False
        emit_event(
            component="channels.qq",
