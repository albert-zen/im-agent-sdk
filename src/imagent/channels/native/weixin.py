...
class WeixinChannelAdapter(BaseChannelAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        transport_factory: Callable[[WeixinCredentials], Any] | None = None,
        ...
        self.outbound_media_dir = Path(
            outbound_media_dir or Path(".imagent") / "outbound-media"
        ).resolve()
    ...
    def from_config(cls, *, config: dict[str, object], middleware):
        ...
        return cls(
            ...
            media_dir=Path(str(config.get("media_dir") or Path(state_dir) / "inbound-media")),
    ...
    async def start(self) -> None:
        ...
        if (
            not self.access_policy.allowed_user_ids
            and not self.access_policy.denies_all
            and credentials.owner_user_id
        ):
    ...
    def _validated_credentials(self) -> WeixinCredentials:
        ...
        if credentials is None:
            raise RuntimeError(
                "Weixin credentials are missing; configure state_dir with "
                "consumer-enrolled credentials"
            )
        return credentials
    ...
    async def handle_raw_message(self, payload: dict[str, Any]) -> None:
        ...
        if image_references or file_references:
            ...
            async def materialize(message):
                return await self._materialize_inbound(
                    message,
                    image_references,
                    file_references,
                )

            prepare_inbound = materialize
    ...
    async def send_message(self, message: OutboundMessage) -> NativeDeliveryResult:
        ...
        if self._auth_stale:
            raise RuntimeError(
                "Weixin credentials are stale; refresh them through the consumer enrollment flow"
            )
        transport = self._transport
        ...
        async def send_artifact(artifact: OutboundArtifact) -> ArtifactDeliveryReceipt:
            try:
                ...
                await transport.send_artifact(
                    ...
                    client_id=(f"imagent-artifact-{identity[:32]}" if identity else None),
                )
                ...
                if exc.code in {413, 415, 422}:
                    raise PermanentArtifactDeliveryError("Weixin rejected the upload") from exc
                raise
            return ArtifactDeliveryReceipt(
                delivery_identity=(f"imagent-artifact-{identity[:32]}" if identity else ""),
            )
    ...
    async def _download_inbound_image(
        ...
        transport = self._transport
        download: Any = getattr(transport, "download_image", None)
        if not callable(download):
            raise MediaDownloadError
        callback = cast(
            Callable[[WeixinImageReference, Callable[[bytes], Awaitable[None]]], Awaitable[None]],
            download,
        )
        await callback(reference, write_chunk)
    ...
    async def _download_inbound_file(
        ...
        transport = self._transport
        download: Any = getattr(transport, "download_file", None)
        if not callable(download):
            raise MediaDownloadError
        callback = cast(
            Callable[[WeixinFileReference, Callable[[bytes], Awaitable[None]]], Awaitable[None]],
            download,
        )
        await callback(reference, write_chunk)
    ...
    def _create_transport(self, credentials: WeixinCredentials) -> Any:
    ...
    def _image_references_from_items(
        ...
        for item in value:
            ...
            references.append(
                WeixinImageReference(
                    encrypted_query_param=cls._string_value(media.get("encrypt_query_param")),
        ...
        for item in value:
            ...
            references.append(
                WeixinFileReference(
                    ...
                    filename=cls._string_value(file_item.get("file_name") or file_item.get("name"))
                    or "file",
