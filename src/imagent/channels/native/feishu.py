...
def _feishu_message_id(result: object) -> str:
    ...
    for candidate in candidates:
        if isinstance(candidate, dict):
            ...
            message_id = getattr(candidate, "message_id", "") or getattr(
                candidate,
                "messageId",
                "",
            )
...
class FeishuChannelAdapter(BaseChannelAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self.media_materializer = media_materializer or ImageMediaMaterializer(
            root=media_dir or Path(".imagent") / "channels" / "feishu" / "inbound-media",
    ...
    def from_config(cls, *, config: dict[str, object], middleware):
        ...
        return cls(
            ...
            startup_timeout_s=_config_float(config.get("startup_timeout_s"), 30.0),
            media_dir=Path(
                str(config.get("media_dir") or ".imagent/channels/feishu/inbound-media")
            ),
    ...
    def _parse_inbound_message_with_images(
        ...
        placeholder_keys = [
            ...
            if self._is_downloadable_image_key(file_key := match.group(2).strip())
        ]
        ...
        malformed_image_placeholder = any(
            match.group(1).strip().casefold() == "image" and not match.group(2).strip()
            for match in placeholder_matches
        )
        if (
            (content_type == "image" and not image_references) or malformed_image_placeholder
    ...
    async def send_message(self, message: OutboundMessage) -> NativeDeliveryResult:
        ...
        if thread_id and not reply_to:
            raise RuntimeError(
                "Feishu topic delivery requires a persisted inbound message ID for reply routing."
            )
        ...
        async def send_artifact(
            ...
            if hasattr(result, "success") and not bool(getattr(result, "success")):
                ...
                if self._artifact_error_is_permanent(error):
                    raise PermanentArtifactDeliveryError("Feishu rejected the upload")
                raise RuntimeError("Feishu temporarily rejected an outbound artifact.")
            return ArtifactDeliveryReceipt(
                platform_message_id=_feishu_message_id(result),
            )
        ...
        message_ids = list(delivered_artifact_message_ids(message))
        ...
        for index, chunk in enumerate(split_text(message.text, limit=FEISHU_TEXT_LIMIT)):
            ...
            if delivery_id:
                opts["uuid"] = hashlib.sha256(
                    f"{delivery_id}\0text\0{index}\0{chunk}".encode()
                ).hexdigest()[:50]
    ...
    def _create_sdk(self) -> Any:
        ...
        return FeishuChannel(
            ...
            # The SDK Channel boundary owns stable inbound message dedup.
    ...
    def _subscribe_sdk(self, sdk: Any) -> None:
        ...
        *,
        ...
        for resource in resources:
            ...
            if (
                ...
                continue
            seen.add(file_key)
            references.append(FeishuImageReference(message_id=message_id, file_key=file_key))
    ...
    async def _disconnect_sdk(self, sdk: Any) -> None:
    ...
    @staticmethod
    def _connection_snapshot(sdk: Any) -> dict[str, object]:
        try:
