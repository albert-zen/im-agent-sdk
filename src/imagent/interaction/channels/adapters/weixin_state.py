...
class WeixinStateStore:
    ...
    def load_credentials(self) -> WeixinCredentials | None:
        ...
        if (
            ...
            raise RuntimeError(
                f"Invalid Weixin credential file: {self.credentials_path}. "
                "Refresh it through the consumer enrollment flow."
            )
    ...
    def _validate_private_path(path: Path, *, expected_mode: int) -> None:
        ...
        if mode & 0o077:
            raise RuntimeError(
                f"Insecure Weixin state permissions on {path}: {mode:o}; "
