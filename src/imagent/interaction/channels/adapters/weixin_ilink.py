...
BASE_INFO = {
    ...
    "bot_agent": f"im-agent-sdk/{__version__}",
}
...
class WeixinILinkTransport:
    ...
    async def send_text(
        ...
        *,
        ...
        message_client_id = client_id or f"imagent-weixin-{uuid.uuid4().hex}"
        ...
        *,
        ...
        if upload_full_url.strip():
            try:
                ...
                raise ILinkError(
                    "iLink getuploadurl returned an invalid upload target", code=400
                ) from None
    ...
    async def _upload_cdn(self, url: str, ciphertext: bytes) -> str:
        for attempt in range(1, 4):
            try:
                ...
                if attempt >= 3:
                    raise ILinkError(f"iLink CDN upload failed ({type(exc).__name__})") from None
        ...
        try:
            async with self.http_client.stream(
                ...
                async for chunk in response.aiter_raw():
                    ...
                    decrypt_size = max(
                        0,
                        ((len(pending) - AES.block_size) // AES.block_size) * AES.block_size,
                    )
                    if decrypt_size:
...
def _image_download_url(reference: WeixinImageReference) -> str:
    ...
    if not query_param or any(character.isspace() for character in query_param):
        raise MediaDownloadError
    return (
        f"{DEFAULT_ILINK_CDN_BASE_URL}/download?encrypted_query_param={quote(query_param, safe='')}"
    )


def _encrypt_outbound_artifact(content: bytes, aes_key: bytes) -> bytes:
    return AES.new(aes_key, AES.MODE_ECB).encrypt(pad(content, AES.block_size, style="pkcs7"))
