...
class AppServerClient:
        ...
        async with self._owned_initialize_lock():
            ...
            if not self._protocol_initialized:
                try:
                    ...
                    if (
                        self.connection_epoch == initialize_epoch
                        and self._transport is initialize_transport
                    ):
        ...
        **kwargs: Any,
        ...
        try:
            ...
            emit_event(
                ...
                level="WARNING",
                message=(
                    "App-server does not support lightweight thread resume; retrying compatibly"
                ),
    ...
    async def list_permission_profiles(
        self, params: JsonDict | None = None, **kwargs: Any
    ) -> JsonDict:
        payload = dict(params or {})
        payload.update(kwargs)
    ...
    def _reply_transport(self, expected_connection_epoch: int | None) -> AppServerTransport:
        ...
        if (
            ...
            raise AppServerError(
                ...
                f"(expected epoch {expected_connection_epoch}, "
                f"current epoch {self.connection_epoch})"
            )
        return transport
    ...
    def _request_uses_local_image_paths(method: str, params: JsonDict | None) -> bool:
        ...
        return bool(
            ...
            and any(
                isinstance(item, dict) and item.get("type") == "localImage" for item in input_items
            )
        )
    ...
    async def _send_json(
        self, payload: JsonDict, *, transport: AppServerTransport | None = None
    ) -> None:
    ...
    async def _dispatch_one(self, message: JsonDict, epoch: int, *, queue_kind: str) -> None:
        ...
        try:
            ...
            if reply_key is None or reply_key in self._answered_server_requests:
                return
            try:
