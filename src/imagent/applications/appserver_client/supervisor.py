...
class MissingAppServerDependencyError(RuntimeError):
    pass


def _websocket_module() -> Any:
    try:
        import websockets
    except ImportError as exc:
        raise MissingAppServerDependencyError(
            "WebSocket App Server targets require the 'appserver' optional dependency"
        ) from exc
    return websockets
...
def _validate_unix_endpoint(endpoint_url: str) -> None:
    if endpoint_url.lower().startswith(UNIX_ENDPOINT_PREFIX) and not _is_unix_endpoint(
        endpoint_url
    ):
...
HealthProbe = Callable[
    [list[str], dict[str, str], float], Awaitable[HealthProbeResult] | HealthProbeResult
]
...
class AppServerSupervisor:
    ...
    process_shutdown_timeout_s: float = 5.0
    ...
    def __post_init__(self) -> None:
        if self.process_shutdown_timeout_s <= 0:
            raise ValueError("process_shutdown_timeout_s must be positive")
    ...
    async def stop(self) -> None:
        ...
        exited = getattr(process, "returncode", None) is not None
        if not exited:
        ...
        if not exited and callable(wait):
            try:
                result = wait()
                if inspect.isawaitable(result):
                    await asyncio.wait_for(
                        result,
                        timeout=self.process_shutdown_timeout_s,
                    )
                exited = True
            except TimeoutError:
                ...
                if not callable(kill):
                    raise RuntimeError(
                        "App Server process did not stop and cannot be killed"
                    ) from None
                try:
                    kill()
                except ProcessLookupError:
                ...
                if not exited:
                    ...
                    if inspect.isawaitable(result):
                        try:
                            ...
                            raise RuntimeError(
                                "App Server process did not exit after kill"
                            ) from exc
                    exited = True
            except ProcessLookupError:
                exited = True
    ...
    async def _connect_with_retries(
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                return connection
            except MissingAppServerDependencyError:
                raise
            except Exception as exc:
                self._record_websocket_connect_failure(
                    url=url, attempt=attempt, attempts=attempts, exc=exc
                )
                if attempt >= attempts:
                    return None
    ...
    def _open_websocket(
        ...
        if _is_unix_endpoint(url):
            ...
            connect_unix = self.unix_websocket_factory or _websocket_module().unix_connect
            kwargs: dict[str, Any] = {
    ...
    async def _probe_health(self, url: str, headers: dict[str, str]) -> HealthProbeResult | None:
        ...
        emit_event(
            component="appserver.supervisor",
            event="appserver.connect.health_probe_succeeded"
            if result.ok
            else "appserver.connect.health_probe_failed",
            level="INFO" if result.ok else "WARNING",
            message="App-server health probe succeeded"
            if result.ok
            else "App-server health probe failed",
...
async def _default_connect(url: str, **kwargs: Any) -> Any:
    return await _websocket_module().connect(url, **kwargs)
