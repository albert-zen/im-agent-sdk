...
def summarize_transport_message(
    message: dict[str, Any], *, max_preview_chars: int = 240
) -> dict[str, Any]:
    ...
    if "message" in payload:
        summary["message_preview"] = _trim_preview(
            str(payload.get("message") or ""), max_preview_chars
        )
...
def _redact_managed_media(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {
            _redact_managed_media(key): _redact_managed_media(item) for key, item in value.items()
        }
    return value
